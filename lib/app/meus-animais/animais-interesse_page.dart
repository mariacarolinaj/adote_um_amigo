...
class AnimaisInteressePage extends StatefulWidget {
  final String title;

  const AnimaisInteressePage({Key? key, this.title = 'MeusAnimaisPage'})
      : super(key: key);

  @override
  State<AnimaisInteressePage> createState() => _AnimaisInteressePageState();
}

class _AnimaisInteressePageState extends State<AnimaisInteressePage> {
  ...
  initState() {
    ...
    DataBaseService().getUsuarioLogado().then((result) {
      _user = result;
      DataBaseService().getInteressesByUserId(_user.id).then((value) {
        _animais = value;
        setState(() {});
      });
    });
  ...
  Widget _buildTitleHeader() {
    return Center(
      child: RichText(
        text: const TextSpan(
          text: 'Meus animais de interesse',
          style: TextStyle(
              color: Cores.titulo,
              fontSize: 32,
              fontFamily: 'Jost',
